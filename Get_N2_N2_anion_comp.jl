...
for i in 0:302
    local file_name = "Test Data/Gaussian Data/Anion/N2+N2/N2_N2_"*string(i)*".log"
...
N2_N2_ecp_model_data .-= N2_N2_ecp_model_data[1:3:end][end];
N2_N2_CCSDT_data .-= N2_N2_CCSDT_data[1:3:end][end];

N2_N2_ecp_model_data = N2_N2_ecp_model_data[isinf.(N2_N2_CCSDT_data) .== false];
N2_N2_CCSDT_data = N2_N2_CCSDT_data[isinf.(N2_N2_CCSDT_data) .== false];

N2_N2_ecp_model_data .*= kjmol;
N2_N2_CCSDT_data .*= kjmol;

aux_X_dat = N2_N2_CCSDT_data[N2_N2_CCSDT_data .< 400];
aux_Y_dat = N2_N2_ecp_model_data[N2_N2_CCSDT_data .< 400];

aux_Y_dat = aux_Y_dat[aux_X_dat .> 50];
aux_X_dat = aux_X_dat[aux_X_dat .> 50];
...
N2_N2_P2 = scatter(N2_N2_CCSDT_data,N2_N2_ecp_model_data,label=false);
plot!(xlims=(X0,X1),ylims=(Y0,Y1));
# plot!(xticks=0:0.1:0.3,yticks=0:0.1:0.3);
plot!(legend=:topleft);
plot!([X0,X1],[X0*X[1]+X[2],X1*X[1]+X[2]],label=aux_label);

px = X0 + 0.875*(X1-X0);
py = Y0 + 0.1*(Y1-Y0);
annotate!(px,py,text("(N₂ + N₂)⁻",:center,:center,10));
...
fp2 = @pgf Axis(
    {
        ...
        yticklabels = L"$0$,$250$,$500$,$750$,$1000$",
    ...
    Plot(
        ...
        "node[] at (650,100) {\$ \\left( \\mathrm{N}_2 + \\mathrm{N}_2 \\right)^- \$}",
    ),
    Plot(
        {
            no_markers,
