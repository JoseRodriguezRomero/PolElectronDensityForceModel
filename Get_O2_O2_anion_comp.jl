...
for i in 0:302
    local file_name = "Test Data/Gaussian Data/Anion/O2+O2 (2S+1=4)/O2_O2_"*string(i)*".log"
...
mol_ecp_a.charge = -0.5;
PolarizeMolecules!([mol_ecp_a],xc_coeffs_ECP_Pol);
mol_ecp_e0 = NaiveEnergyFromDensity([mol_ecp_a]);
mol_ecp_e0 += (XCEnergyFromDensity([mol_ecp_a],order)*xc_coeffs_ECP)[1];
...
O2_O2_ecp_model_data .-= O2_O2_ecp_model_data[1:3:end][end];
O2_O2_CCSDT_data .-= O2_O2_CCSDT_data[1:3:end][end];
...
O2_O2_ecp_model_data .*= kjmol;
O2_O2_CCSDT_data .*= kjmol;

M = hcat([minimum(O2_O2_CCSDT_data),1.0],[maximum(O2_O2_CCSDT_data),1.0])';
Y = [minimum(O2_O2_ecp_model_data),maximum(O2_O2_ecp_model_data)];
X = M \ Y;
...
O2_O2_P2 = scatter(O2_O2_CCSDT_data,O2_O2_ecp_model_data,label=false);
plot!(xlims=(X0,X1),ylims=(Y0,Y1));
plot!(xticks=0:500:2000,yticks=0:900:4500);
...
px = X0 + 0.875*(X1-X0);
py = Y0 + 0.1*(Y1-Y0);
annotate!(px,py,text("(O₂ + O₂)⁻",:center,:center,10));
...
fp3 = @pgf Axis(
    {
        ...
        xticklabels = L"$0$,$500$,$1000$,$1500$,$2000$",
        yticklabels = L"$0$,$900$,$1800$,$2700$,$3600$,$4500$",
    ...
    Plot(
        ...
        "node[] at (1625,450) {\$ \\left( \\mathrm{O}_2 + \\mathrm{O}_2 \\right)^- \$}",
    ),
    Plot(
        {
            no_markers,
